...
async function mount(root?: string, threadIdleMs = 60000, onExpired: (binding: ThreadBinding) => void = () => {}) {
  ...
  const binding = new ThreadBinding(ctx, { tenantId: 'tenant', userId: 'user' }, 'thread-1', SessionId('native-content'), {
    provider: 'scripted', model: 'scripted', workspaceRoot: join(directory, 'workspaces'), fileSecret: 'test-only-upload-signing-secret',
    frontendToolTimeoutMs: 1000, threadIdleMs, maxRunEvents: 128, maxRunEventBytes: 128 * 1024,
...
describe('native file receipt admission', () => {
  ...
  it('does not dispatch when the client leaves while file admission is pending', async () => {
    const { ctx, binding } = await mount()
    const { source } = await upload(binding)
    let release!: () => void
    const gate = new Promise<void>(resolve => { release = resolve })
    const admit = ctx.attachments.admitPromptContent.bind(ctx.attachments)
    vi.spyOn(ctx.attachments, 'admitPromptContent').mockImplementationOnce(async parts => { await gate; return admit(parts) })
  ...
  it('fails clearly when a text-only host has no native files or signing configuration', async () => {
    ...
    const binding = new ThreadBinding(ctx, { tenantId: 't', userId: 'u' }, 't', SessionId('text-only'), {
      provider: 'scripted', model: 'scripted', workspaceRoot: '/unused', frontendToolTimeoutMs: 1000,
