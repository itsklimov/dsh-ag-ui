...
describe('resumed threads keep their recorded composition', () => {
  ...
  it('restores a native selected preset after restart even when the creation default differs', async () => {
    ...
    const original = new ThreadBinding(first, principal, 'preset-resume', sessionId, OPTIONS('alpha', join(root, 'workspaces')), () => {})
    ...
    const resumed = new ThreadBinding(second, principal, 'preset-resume', sessionId, OPTIONS('alpha', join(root, 'workspaces')), () => {})
...
describe('thread preset selection', () => {
  ...
  it('rejects malformed selectors and invalid work before changing the composition', async () => {
    ...
    const invalid = await post(url, 'invalid', 'beta', [{ id: 'bad', role: 'user', content: 'First' }, { id: 'bad', role: 'user', content: 'Duplicate' }])
...
describe('preset selection admission lifetime', () => {
  ...
  async function bindingFor(withRoster = true) {
    ...
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'ag-ui-selection-'))
    roots.push(workspaceRoot)
    const binding = new ThreadBinding(ctx, PRINCIPAL, input.threadId, durableSessionId(PRINCIPAL, input.threadId, SECRET), {
      workspaceRoot, maxFilesPerMessage: 8, provider: 'scripted', model: 'scripted', ...(withRoster ? { presetId: 'alpha' } : {}), selectablePresetIds: new Set(['beta']),
