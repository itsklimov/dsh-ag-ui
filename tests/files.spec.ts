...
async function mount(overrides: Partial<Config> = {}, persistenceRoot?: string, nativeHome?: string) {
  ...
  return { ctx, workspaceRoot, nativeRoot, url: `http://127.0.0.1:${String(ctx.webServer.port)}${overrides.path ?? '/ag-ui'}` }
}
...
describe('native thread file HTTP routes', () => {
  it('streams exact bytes and native hash metadata with authenticated downloads', async () => {
    ...
    expect(await readdir(workspacePath(mounted.workspaceRoot))).toEqual([])
  }, 15000)
  ...
  it('downloads a durable upload after a cold restart without a gateway files index', async () => {
    ...
    roots.push(root)
    const overrides = { workspaceRoot: join(root, 'workspaces') }
    const first = await mount(overrides, join(root, 'sessions'), join(root, 'native'))
    const source = await (await upload(first.url, 'durable')).json() as { value: string }
    await first.ctx.fiber.dispose()
    contexts.splice(contexts.indexOf(first.ctx), 1)
    const second = await mount(overrides, join(root, 'sessions'), join(root, 'native'))
